...
def after_install(options, home_dir):
    ...
    try:
        cmd = [os.path.abspath(join(home_dir, script_dir, 'python')),
               ...
               '--record=.\\tmp\\appengine-tmp-record.txt',
    ...
    import glob
    filename = glob.glob(os.path.join(home_dir, 'lib/site-packages/*/pkg_resources.py'))[0]
...
def fixup_distutils_cfg(options, home_dir):
    ...
    f.write('''# This makes installation work properly with pip
[easy_install]
zip_ok = False
...
def install_runner(options, home_dir):
    ...
    if not os.path.exists(conf):
        f = open(conf, 'w')
        f.write('''APP_NAME = '%s.wsgiapp:make_app'
APP_ARGS = ()
APP_KWARGS = dict()
